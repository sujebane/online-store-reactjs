...
const Landing = () => {
  const button = [
    {buttonname:"Get Started", styles:"border-solid rounded-full border-2 p-2 px-8 bg-white text-black whitespace-nowrap", onClick:"/getstarted"},
    {buttonname:"Lern more", styles:"whitespace-nowrap", onClick:"/lernmore"},
  ];
  return (
    ...
    <div className='flex items-stretch gap-x-96 mx-96'>{button.map((button)=><Button buttonname={button.buttonname} styles={button.styles} onClick={button.onClick}/>)}</div>
    <Image/>
    <StatBar/>
    <br></br>
    </div>
